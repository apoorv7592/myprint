...
function getcards(page_no){
	...
	dataType:"json",
	async:"TRUE",
	url:"<%= api_discover_path(@discover)%>",
	method:"GET",
	data:{cardnum:$(".card").length,range:4*colnum, page:page_no},
	beforeSend:function(){$("#loaderarea").show();},
	success:function(data){
		cardsdata.push.apply(cardsdata,data);
		for(var i=0 ; i<data.length ; i++){
			var card = "<div id=\""+data[i]['id']+"\" class=\"card\"><div class=\"cardimage\" type=\"button\" class=\"btn\" data-toggle=\"modal\" data-target=\"#myModal\"><img src=\""+data[i]['spree_assets'][0]['url']+"\"></div><div class=\"cardpanel\"><button class=\"buyitnow\"><strong>Buy Now</strong></button><div class=\"pricing\">Rs."+ data[i]['price']+"</div><div class=\"cardinfo\">"+data[i]['short_descrip']+"</div><div class=\"designer_info\"><a href=/designers/"+data[i]['designer']['slug']+" class=\"btn bttn\" style=\"padding:0;background:white;color:#5F5F5F\"><image src=\""+data[i]['designer']['img']+"\" style=\"height:35px;\" class=\"img-circle\" > "+data[i]['designer']['name']+"</a></div></div><button class=\"btn addtowishlist bttn\"><i class=\"fa fa-heart fa-1x\" style=\"position:relative;left:-6px;top:-3px\"></i></button></div>";
...
function createmodal(x){
	...
	success:function(data){
		...
		$(".modal-image").attr("src",$(".selected img").attr("src"));
		$(".designer_name").html("Designer Name");
		$(".protitle").html("<strong>"+data1['name']+"</strong>");
		$(".proprice").html("<strong>Rs. "+data1['price']+ "</strong>");
		$(".quantity").html("<select><option>option1</option><option>option2</option><option>option3</option><></select>")
		...
		$(".overview").html("<h4>Overview</h4><ul><li>Handmade item</li><li>Made to order</li><li>Feedback: 161 reviews</li><li>Ships worldwide from Düsseldorf, Germany</li></ul>");
...
function getsubcategories(){
	...
	dataType:"json",
	async:"TRUE",
	url:"<%= api_get_discovers_path(@discover.category)%>",
	type:"GET",
	data:{categoryname:''},
	success:function(data){
...
(function() {
  ...
  (function($) {}, $('#new_wished_product').on('submit', function() {}), selected_variant_id = $('#product-variants input[type=radio]:checked').val(), selected_variant_id ? $('#wished_product_variant_id').val(selected_variant_id) : void 0, $('form#change_wishlist_accessibility').on('submit', function() {}), $.post($(this).prop('action'), $(this).serialize(), null, 'script'), false, $('form#change_wishlist_accessibility input[type=radio]').on('click', function() {}), $(this).parent().submit())(jQuery);
